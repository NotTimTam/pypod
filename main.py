...
if JELLYFIN_URL:
    ...
    if not (JELLYFIN_LIBRARY and JELLYFIN_USERNAME and JELLYFIN_PASSWORD):
        raise ValueError(
            "JELLYFIN_URL configured but missing JELLYFIN_USERNAME, JELLYFIN_PASSWORD and/or JELLYFIN_LIBRARY"
        )
    jellyfin = Jellyfin(
        JELLYFIN_URL,
        JELLYFIN_USERNAME,
...
def load_screen(name, state=None):
    ...
    if name == "home":
        ...
        from src.screens.jellyfin_artists import (
            JellyfinArtistsScreen as _JellyfinArtistsScreen,
        )

        return _JellyfinArtistsScreen(
            state=state,
            request_screen=request_screen,
            jellyfin=jellyfin,
            download_manager=download_manager,
        ...
        from src.screens.jellyfin_albums import (
            JellyfinAlbumsScreen as _JellyfinAlbumsScreen,
        )

        return _JellyfinAlbumsScreen(
            state=state,
            request_screen=request_screen,
            jellyfin=jellyfin,
            download_manager=download_manager,
        ...
        from src.screens.jellyfin_songs import (
            JellyfinSongsScreen as _JellyfinSongsScreen,
        )

        return _JellyfinSongsScreen(
            state=state,
            request_screen=request_screen,
            jellyfin=jellyfin,
            download_manager=download_manager,
...
try:
    while True:
        ...
        if is_sleeping:
            ...
            if input_handler.has_events() or any(
                input_handler.button_state(btn) for btn in ["A", "B", "X", "Y"]
            ):
                update_activity()
                continue
        ...
        if (
            media_player.get_status() != PlayerStatus.STOPPED
            and current_screen_name != "now_playing"
        ):
