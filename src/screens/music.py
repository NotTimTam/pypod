...
class MusicScreen(Screen):
    ...
    def _setup_menu(self):
        ...
        self.menu.add_item("Shuffle Songs", partial(self._media_player.play_all_songs, True))
        self.menu.add_item("Playlists >", self.nullish)
        self.menu.add_item(
            "Artists >", partial(self._request_screen, "artists", {"return_index": 2})
        )
        self.menu.add_item(
            "Albums >",
            partial(
                ...
                "albums",
                {"return_index": 3, "from_artist": False},
            ),
        )
        self.menu.add_item(
            "Songs >",
            partial(
                self._request_screen,
                "songs",
                {"return_index": 4, "return_screen": "music"},
            ),
        )
    ...
    def handle_input(self):
        ...
        input_handler.handle_button(
            "B", lambda: self._request_screen("home", {"menu": {"current_index": 0}})
        )
